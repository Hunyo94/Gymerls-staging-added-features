...
function Reservation() {
  ...
  const [selectedMonthUpdate, setSelectedMonthUpdate] = useState("");
  const [thisMonthUpdate, setThisMonthUpdate] = useState("");
  const [getThisUpdateDate, setGetThisUpdateDate] = new useState(
    dayjs(new Date())
  );
  ...
  const [selectedUpdateTimeSlot, setSelectedUpdateTimeSlot] = useState("");
  ...
  const updateReservation = async (id, date) => {
    fetch("http://localhost:3030/api/get-reservation-by-id", {
      method: "POST",
      headers: {
  ...
  const handleOpenModalCreateReservation = () => {
    ...
    fetch("http://localhost:3030/api/user-membership")
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      fetch(
        // "http://localhost:3030/api/get-reservation-by-date",
        "http://localhost:3030/api/get-reservation-all",
        {
          method: "POST",
          headers: {
            "Content-type": "application/json",
  ...
  const filteredDataByStatusAndDate = (status) => {
    ...
    if (status === "All") {
      ...
      fetch("http://localhost:3030/api/get-reservation-by-date-and-status", {
        method: "POST",
        headers: {
          "Content-type": "application/json",
        },
          ...
          if (selectValidTime >= timeNow) {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                fetch("http://localhost:3030/api/create-reservation", {
                  method: "POST",
                  headers: {
                    "Content-type": "application/json",
                  },
  ...
  const handleMonthsUpdate = (date) => {
    setSelectedUpdateTimeSlot("");
    setUpdateSelectedTimeSlot("");
    let selectMonth = updateResDateForSelected
      .toString()
      .slice(8, 11)
      .toString();
    let month = newDate.slice(8, 11);
    ...
    setSelectedMonthUpdate(selectDate);
  };
  ...
  const handleUpdateReservation = (event) => {
    ...
    const e = monthNow.slice(5, 7);
    const i = Number(e);
    let dayNow = i;
    let selectValidTime = "";
    if (selectedUpdateTimeSlot === "7-9AM") {
      selectValidTime = 1;
    } else if (selectedUpdateTimeSlot === "9-11AM") {
      selectValidTime = 2;
    } else if (selectedUpdateTimeSlot === "1-3PM") {
      selectValidTime = 3;
    } else if (selectedUpdateTimeSlot === "3-5PM") {
      selectValidTime = 4;
    } else if (selectedUpdateTimeSlot === "5-7PM") {
      selectValidTime = 5;
    } else if (selectedUpdateTimeSlot === "7-9PM") {
      selectValidTime = 6;
    } else {
      selectValidTime = 7;
    }
    // console.log(selectDate);
    // console.log(thisMonthUpdate);
    // YEAR VALIDAION
        ...
        if (selectedDay == dayNow) {
          ...
          if (selectValidTime >= timeNow) {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                fetch("http://localhost:3030/api/update-reservation", {
                  method: "PATCH",
                  headers: {
                    "Content-type": "application/json",
                  },
              ...
              <DialogContent>
                ...
                <div>
                  ...
                  <TextField
                    ...
                    onChange={(e) => {
                      handleMonthsUpdate();
                      setSelectedUpdateTimeSlot(e.target.value);
